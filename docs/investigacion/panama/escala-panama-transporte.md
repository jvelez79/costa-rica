# Transporte - PTY ↔ Ciudad de Panamá

**Distancia:** 17-24 km (PTY → Casco Viejo)
**Tiempo:** 20-30 min (sin tráfico) | Hasta 45 min (con tráfico)

---

## ⏰ CONTEXTO: Dos Escalas Diferentes

### 🛫 IDA - Domingo 23 Noviembre
- **Llegada PTY:** 7:30 AM
- **Salida PTY:** 6:28 PM
- **Escala:** **10h 58min** ← Tiempo SUFICIENTE para explorar ciudad
- **Recomendación:** Uber o Metro son viables según presupuesto

### 🛬 REGRESO - Sábado 29 Noviembre
- **Llegada PTY:** 10:32 AM
- **Salida PTY:** 12:21 PM
- **Escala:** **1h 49min** ← Tiempo CORTO, quedarse en terminal
- **Recomendación:** NO salir del aeropuerto

---

## 📊 COMPARACIÓN RÁPIDA (para escala IDA de 10h 58min)

| Opción | Costo (ida) | Tiempo | Dificultad | Recomendado |
|--------|-------------|--------|------------|-------------|
| **Uber** | $13-18 | 20-30 min | Fácil | ⭐⭐⭐⭐⭐ Mejor relación precio/tiempo |
| **Taxi Oficial** | $30 | 20-30 min | Fácil | ⭐⭐⭐ OK si no tienes app |
| **Metro + Metrobús** | $0.75 | 45-70 min | Media | ⭐⭐⭐⭐ Excelente si hay presupuesto ajustado |
| **Private Transfer** | $40-60 | 20-30 min | Muy fácil | ⭐⭐⭐⭐ Premium |
| **Bus Público** | $0.25 | 1+ hora | Difícil | ❌ NO recomendado |

---

## 🥇 OPCIÓN 1: UBER (RECOMENDADO)

### Costo
- **Ida:** $13-18
- **Vuelta:** $13-18
- **Total pareja:** ~$30-36

### Tiempo
- 20-30 min (domingo ligero)
- Hasta 45 min si tráfico

### Cómo Usar desde PTY

**Paso a paso:**
1. Pasar migración (~30 min)
2. Guardar equipaje ($5/maleta)
3. Salir por "Arrivals"
4. **Cruzar calle hacia parking lot**
5. Abrir app Uber
6. Destino: "Casco Viejo, Panama City"
7. Encontrar **Rows 15-18 del parking**
8. Esperar conductor (verifica placa en app)

**⚠️ IMPORTANTE:**
- Uber NO puede recoger en zona Arrivals
- DEBES ir a parking rows 15-18
- Tracking en tiempo real en app

### Regreso (Casco → PTY)
- Mucho más fácil
- Pedir Uber donde estés
- **⏰ Pedir 2:00 PM MÁXIMO**
- Destino: "Tocumen International Airport (PTY)"

### Ventajas
- ✅ Precio transparente
- ✅ Tracking en tiempo real
- ✅ Más económico que taxi ($17 vs $35)
- ✅ Pago con tarjeta en app
- ✅ Sin regateo

---

## 🚕 OPCIÓN 2: TAXI OFICIAL

### Costo (Tarifas Oficiales 2025)
- **Tarifa fija Casco Viejo:** $30.00 (1-2 personas)
- **Adicional:** $10-20 por pasajero extra (dependiendo distancia)
- **Total roundtrip pareja:** ~$60

**Fuente:** [panamatocumen.com/taxis](https://panamatocumen.com/taxis) (sitio oficial)

### Desde PTY
1. Salir de zona **Arrivals**
2. Buscar **taxi stand oficial** en área de transporte terrestre
3. Ir al **mostrador autorizado** (counter oficial)
4. Decir destino: "Casco Viejo"
5. Pagar **$30** en efectivo (USD)
6. Te asignan taxi amarillo oficial
7. **NO pagar nuevamente al conductor** (ya pagaste en counter)

### ⚠️ ADVERTENCIAS IMPORTANTES

**Riesgo de sobrecargos:**
- ❌ Conductores NO oficiales se acercan agresivamente a turistas
- ❌ Intentan cobrar tarifas infladas (más de $30)
- ❌ Dicen ser "oficiales" pero no lo son

**Protégete:**
- ✅ **SOLO usar taxis del counter oficial**
- ✅ **Confirmar precio ANTES de cargar equipaje**
- ✅ Ignorar conductores que te aborden directamente
- ✅ Si te dicen precio mayor a $30 a Casco Viejo = NO es oficial

### Regreso (Casco → PTY)
- Negociar precio ANTES de subir ($25-30)
- O pedir al restaurante que llamen taxi oficial/confiable
- Confirmar tarifa antes de arrancar

### Método de Pago
- **Efectivo SOLAMENTE** (USD preferido)
- **NO aceptan tarjeta** (muy raro que acepten)
- Llevar billetes pequeños ($20, $10, $5)

### Ventajas
- ✅ No requiere smartphone/app/internet
- ✅ Oficial, regulado (si usas counter oficial)
- ✅ Disponible 24/7
- ✅ Inmediatamente disponible

### Desventajas
- ❌ Solo efectivo (no tarjeta)
- ❌ Sin tracking GPS
- ❌ Riesgo de conductores NO oficiales que intentan estafar
- ❌ Doble precio que Uber ($30 vs $15-18)

---

## 🚇 OPCIÓN 3: METRO + METROBÚS

### Costo (Actualizado 2025)
- **Tarjeta prepago:** $2 (primera vez, recargable)
- **Metrobús (PTY → Corredor Sur):** $0.25
- **Metro (Línea 2):** $0.50
- **Total ida:** $0.75
- **Total roundtrip:** ~$3.50 (incluye tarjeta)

### Tiempo Total
- 45-70 min (transporte + esperas + caminatas)
- Vs 20-30 min en Uber/taxi

---

### 🗺️ Sistema del Metro de Panamá

**Líneas operativas:**
- **Línea 1 (Roja):** Albrook ↔ San Isidro (atraviesa centro ciudad)
- **Línea 2 (Azul):** San Miguelito ↔ Nuevo Tocumen
- **Línea 3:** En construcción (nodo Albrook)

**Estación Albrook:**
- Nodo principal de integración
- Conexión con terminal de buses
- Transfer entre Línea 1 y futura Línea 3

---

### 🛤️ Ruta Completa: PTY → Casco Viejo

**PASO A PASO ACTUALIZADO:**

1. **En Aeropuerto PTY:**
   - Salir de zona de llegadas
   - Buscar parada de **Metrobús**
   - Línea hacia **Corredor Sur**

2. **Comprar tarjeta prepago:**
   - Costo: $2 en puntos autorizados/estaciones
   - Recargar con mínimo $1
   - También acepta Visa/Mastercard contactless en torniquetes
   - **IMPORTANTE:** No se acepta efectivo en torniquetes

3. **Tomar Metrobús ($0.25):**
   - Desde PTY hacia **Estación Corredor Sur** (Línea 2 del Metro)
   - Tiempo: ~10-15 min
   - Pasar tarjeta al subir

4. **Bajar en Corredor Sur**
   - Entrar a estación del Metro Línea 2

5. **Tomar Metro Línea 2 ($0.50):**
   - Dirección: **San Miguelito**
   - Pasar tarjeta en torniquete
   - Viajar ~15-20 min

6. **Transfer en San Miguelito:**
   - Bajar Línea 2
   - Caminar 3-5 min dentro de estación
   - Encontrar Línea 1 (Roja)

7. **Tomar Metro Línea 1:**
   - Dirección: **Albrook**
   - Bajar en: **Estación 5 de Mayo** (más cercana a Casco Viejo)
   - Tiempo: ~10-15 min

8. **Caminar a Casco Viejo:**
   - Desde 5 de Mayo: 15-20 min a pie
   - Usar Google Maps
   - Casco Viejo NO tiene estación propia aún

**REGRESO:** Ruta inversa
- 5 de Mayo → San Miguelito (Línea 1)
- Transfer a Línea 2
- San Miguelito → Corredor Sur (Línea 2)
- Metrobús: Corredor Sur → PTY

---

### ⏰ Horarios Metro (2025)

- **Lunes-Viernes:** 5:00 AM - 11:00 PM
- **Sábados:** 5:00 AM - 10:00 PM
- **Domingos:** 7:00 AM - 10:00 PM
- **⚠️ Domingo 23 Nov:** Abre 7:00 AM

**Frecuencia:**
- Hora pico: Cada 3-5 min
- Hora valle: Cada 5-10 min
- Domingos: Cada 7-12 min

---

### Ventajas
- ✅ MUY económico ($0.75 ida vs $15-35 Uber)
- ✅ Sin tráfico (tren)
- ✅ Moderno, limpio, aire acondicionado
- ✅ Seguro y bien señalizado
- ✅ Sistema integrado Metro + Metrobús con misma tarjeta

### Desventajas
- ❌ **Tiempo:** 45-70 min vs 20-30 Uber
- ❌ Requiere transfer Metrobús → Metro Línea 2 → Metro Línea 1
- ❌ Caminar 15-20 min desde estación 5 de Mayo
- ❌ **Pierdes 1.5-2 horas de tiempo en ciudad** (ida+vuelta)
- ❌ Incómodo con equipaje (múltiples transfers)
- ❌ Complejidad para turistas sin experiencia
- ❌ Riesgo de perderse/confundirse con transfers

---

### 🎯 Notas Importantes

**Tarjeta prepago:**
- Obligatoria para Metro y Metrobús
- Recargable en estaciones y puntos autorizados
- No se acepta efectivo en torniquetes
- Misma tarjeta sirve para todo el sistema

**Conexión aeropuerto:**
- El Metro NO llega directo al aeropuerto
- Requiere combinar Metrobús + Metro Línea 2
- Señalización clara en aeropuerto

**Estación más cercana a Casco Viejo:**
- 5 de Mayo (Línea 1)
- Desde ahí: 15-20 min caminando
- También puedes bajar en otras estaciones más cercanas según destino específico

---

### ✅ Recomendación para Escala IDA (10h 58min)

**Metro + Metrobús ES VIABLE para escala larga:**
- ✅ Con casi 11 horas de escala (10h 58min), pierdes ~2 horas transporte = quedan **8+ horas útiles**
- ✅ Ahorro significativo: $27-30 vs Uber ($5 vs $30-36)
- ✅ Tiempo suficiente para explorar tranquilamente
- ✅ Experiencia local auténtica usando transporte público

**Usar Metro si:**
- Presupuesto ajustado (ahorras $25-30 por pareja)
- Te gusta usar transporte público local
- No te molesta cargar equipaje en transfers
- Quieres experiencia más auténtica

**Usar Uber si:**
- Prefieres comodidad y velocidad
- Llegas cansado del vuelo temprano
- Quieres maximizar tiempo en Casco Viejo (30 min extra por trayecto)
- Viajan con mucho equipaje

**⚠️ IMPORTANTE REGRESO (1h 49min):**
NO salir del aeropuerto - tiempo muy corto, quedarse en terminal

---

## 🚗 OPCIÓN 4: PRIVATE TRANSFER

### Costo
- **Ida:** $40-60
- **Roundtrip:** $80-120

### Cómo Reservar
**ANTES del viaje:**
- Plataformas: Viator, GetYourGuide, Welcome Pickups
- Ingresar: Fecha, hora, vuelo, destino
- Pagar online
- Confirmación por email

**Día del viaje:**
- Conductor rastrea vuelo automáticamente
- Espera con letrero con tu nombre
- Te ayuda con equipaje
- Vehículo privado solo para ti
- Directo a destino

### Ventajas
- ✅ Máxima comodidad
- ✅ Te esperan con letrero
- ✅ Tracking de vuelo (esperan si delay)
- ✅ Vehículo privado
- ✅ Precio fijo (pagado online)

### Desventajas
- ❌ Más caro ($40-60 vs $15-18 Uber)
- ❌ Requiere reserva anticipada
- ❌ Menos flexible (hora fija para regreso)

---

## 🚌 OPCIÓN 5: BUS PÚBLICO

### ❌ NO RECOMENDADO

**Por qué:**
- Línea E489 NO va directo a Casco Viejo
- Requiere múltiples transfers
- Muy lento (1+ hora)
- Difícil con equipaje
- Poco tiempo útil en ciudad

**Si quieres opción económica:** Usar Metro (Opción 3)

---

## 🚦 TRÁFICO DOMINGO

**Buenas noticias:**
- ✅ Domingo = tráfico MUY ligero
- ✅ 10 AM-2 PM = fuera de rush hours
- ✅ Ruta PTY-Casco: Corredor Sur (highway fluido)

**Rush hours típicos (evitar):**
- Lun-Vie 7-9 AM: Pesado hacia ciudad
- Lun-Vie 5-7 PM: Pesado saliendo ciudad
- Sáb/Dom: Tráfico ligero todo el día

**Tiempo esperado domingo:**
- 10 AM PTY → Casco: 20-25 min
- 2 PM Casco → PTY: 20-30 min

---

## ⏰ TIMING PARA ESCALA IDA (10h 58min)

### Timeline Sugerido - CON UBER (más rápido)

**7:30 AM** - Aterrizaje PTY
**7:45-8:15 AM** - Migración (~30 min)
**8:15-8:30 AM** - Guardar equipaje ($5/maleta)
**8:30-9:00 AM** - Salir terminal, pedir Uber
**9:00-9:30 AM** - **Viaje PTY → Casco** (Uber, 20-30 min)

**9:30 AM-4:30 PM** - **~7 HORAS en Casco Viejo** ✨

**4:30 PM** - ⚠️ **Salir de Casco Viejo - Pedir Uber regreso**
**4:30-5:15 PM** - Viaje Casco → PTY (30 min + buffer)
**5:15 PM** - Llegar PTY
**5:15-6:00 PM** - Recoger equipaje, pasar seguridad, ir al gate
**6:00 PM** - En gate, listos para boarding
**6:28 PM** - **Salida vuelo CM 460 a San José**

---

### Timeline Sugerido - CON METRO (más económico)

**7:30 AM** - Aterrizaje PTY
**7:45-8:15 AM** - Migración (~30 min)
**8:15-8:30 AM** - Guardar equipaje ($5/maleta)
**8:30-9:00 AM** - Encontrar Metrobús, comprar tarjeta
**9:00-10:15 AM** - **Viaje PTY → Casco** (Metrobús + Metro, ~70 min)

**10:15 AM-4:00 PM** - **~6 HORAS en Casco Viejo** ✨

**4:00 PM** - ⚠️ **Salir de Casco Viejo hacia Metro**
**4:00-5:15 PM** - Viaje Casco → PTY (Metro + Metrobús, ~70 min)
**5:15 PM** - Llegar PTY
**5:15-6:00 PM** - Recoger equipaje, pasar seguridad, ir al gate
**6:00 PM** - En gate, listos para boarding
**6:28 PM** - **Salida vuelo CM 460 a San José**

**Diferencia:** Uber = 7 horas en ciudad | Metro = 6 horas en ciudad (1 hora menos)

---

## 💰 COSTO TOTAL TRANSPORTE (PAREJA)

| Opción | Costo Total | Tiempo Perdido | Horas en Ciudad |
|--------|-------------|----------------|-----------------|
| Uber | $30-36 | 1 hora | ~7 horas |
| Taxi Oficial | $60 | 1 hora | ~7 horas |
| Metro + Metrobús | $5.00 | 2+ horas | ~6 horas |
| Private Transfer | $80-120 | 1 hora | ~7 horas |

---

## 🏆 RECOMENDACIÓN FINAL (Escala IDA 10h 58min)

### 🥇 OPCIÓN BALANCEADA: UBER
**Mejor para:** Comodidad + Maximizar tiempo en ciudad
- ✅ $30-36 roundtrip pareja
- ✅ Rápido (20-30 min cada trayecto)
- ✅ **7 horas útiles en Casco Viejo**
- ✅ Fácil de usar, sin complicaciones
- ✅ Ideal si llegas cansado o con equipaje pesado

### 🥈 OPCIÓN ECONÓMICA: METRO + METROBÚS
**Mejor para:** Presupuesto ajustado + Experiencia local
- ✅ $5.00 roundtrip pareja (**ahorro de $25-30 vs Uber**)
- ✅ **6 horas útiles en Casco Viejo** (solo 1 hora menos que Uber)
- ✅ Experiencia auténtica de transporte local
- ✅ Sistema moderno, limpio y seguro
- ⚠️ Requiere múltiples transfers (Metrobús → Línea 2 → Línea 1)
- ⚠️ Más cansado con equipaje de mano

### 🥉 ALTERNATIVA: TAXI OFICIAL
**Mejor para:** Sin smartphone o no confías en apps
- $60 roundtrip (tarifa oficial $30 cada trayecto)
- Misma velocidad que Uber
- Sin necesidad de tecnología o internet
- ⚠️ Solo efectivo, no tarjeta
- ⚠️ Cuidado con conductores NO oficiales que cobran más
- ❌ Doble precio que Uber ($60 vs $30-36)

### 💎 PREMIUM: PRIVATE TRANSFER
**Mejor para:** Máxima comodidad sin preocupaciones
- $80-120 roundtrip
- Te esperan con letrero, ayudan con equipaje
- ❌ Precio alto para escala de exploración

---

## ✅ VEREDICTO

**Con 10h 58min de escala, AMBAS opciones son viables:**

- **Elige Uber si:** Valoras comodidad, velocidad, y 1 hora extra en ciudad ($30-36)
- **Elige Metro si:** Priorizas ahorro significativo y experiencia local ($5)

**Ambas te dan suficiente tiempo (6-7 horas) para:**
- Explorar Casco Viejo
- Almorzar tranquilamente
- Visitar 3-4 atracciones principales
- Compras de souvenirs
- Regresar con tiempo de sobra al aeropuerto

**⚠️ RECUERDA:** En el regreso (29 Nov) la escala es solo 1h 49min - **NO salir del aeropuerto**
